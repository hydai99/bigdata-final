...
class BFR(object):
	...
	def first_round(self, data, n_clusters):
		kmeans = KMeans(n_clusters=5*n_clusters, random_state=2023).fit(data[:,2:])
		RS = self.find_RS(clusters=kmeans.labels_)
		data, RS = self.split_RS(data=data, single_clusters=RS)
		kmeans = KMeans(n_clusters=n_clusters, random_state=2023).fit(data[:,2:])
	...
	def inter_round(self, data, DS, CS_c, CS_p, RS, dps, n_clusters):
		...
		if len(RS) > 5*n_clusters:
			kmeans = KMeans(n_clusters=5*n_clusters, random_state=2023).fit(RS[:,2:])
...
def main():
	...
	parser.add_argument("input", default="data.txt", help="input yelp file")
	...
	conf = SparkConf().setAppName("my_app").setMaster("local[*]").set("spark.driver.memory", "20g")
	sc = SparkContext(conf=conf)
	#sc = SparkContext(conf=conf)
	#sc = SparkSession.builder.master("local[*]").config("spark.driver.memory", "20g").getOrCreate()
	sc.setLogLevel("ERROR")

	start_time = time.time()
	n_clusters = int(args.n_cluster)
	bfr = BFR(num_partition=50)
